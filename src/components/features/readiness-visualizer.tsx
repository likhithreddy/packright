  ...
  return (
    <div className={cn('w-full space-y-1.5', className)}>
      {showLabel && (
        <div className="flex justify-between items-end text-sm">
          ...
          <span className={cn(
            ...
            percentage < 100 ? 'text-yellow-600' : 'text-emerald-600'
          )}>
            {percentage}%
          </span>
        </div>
      ...
      <Progress 
        ...
        indicatorClassName={cn('transition-all duration-500 ease-in-out h-full w-full', getProgressColor(percentage))}
      />
    </div>
  );
}
