...
- **Commit Frequency**: Commit often (e.g., after creating a component, after writing a test) to build a valid history checkpoint. You must explicitly run `yarn format` and `yarn lint` to ensure strict Prettier compliance and code quality before EVERY commit and before raising a PR. Explicitly commit the moment an issue implementation is completed in full.
